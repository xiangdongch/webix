/*
@license
Webix Mobile Scheduler v.3.4.0
...
/***/ function(module, exports, __webpack_require__) {
	...
	var modLocale = _interopRequireWildcard(_locale);
	
	var _config = __webpack_require__(6);
	
	var modConfig = _interopRequireWildcard(_config);
	
	var _xy = __webpack_require__(7);
	
	var modXY = _interopRequireWildcard(_xy);
	
	var _templates = __webpack_require__(8);
	
	var modTemplates = _interopRequireWildcard(_templates);
	
	var _scheduler_recurring = __webpack_require__(9);
	
	var rec = _interopRequireWildcard(_scheduler_recurring);
	...
	if (!scheduler.locale) scheduler.locale = {};
	...
	webix.ready(function () {
		...
		if (scheduler.locale && scheduler.locale.date) webix.Date.Locale = scheduler.locale.date;
		...
		scheduler.types = {
			...
			multi_day_list: {
				...
				templateColor: function templateColor(obj) {
		...
		var views = [{
	  	...
	  	direction:"top"
			...
			rows: [{
				...
				}, delete_button, { view: "template", height: 20 }]
			}]
		}, {
			id: "form",
			rows: [{
			...
			rows: [{
				...
				id: "dateForm",
				type: "clean",
				rows: [{ view: "calendar", height: 310, timepickerHeight: 35, css: "form_calendar", icons: false, id: "formCalendar", width: "auto", calendarTime: config.calendar_hour, timepicker: 1, hourStart: 0 }, { view: "template", css: "form_template" }]
			}]
		}].concat(config.views);
		...
		webix.protoUI({
			...
			_setDateMethods: function _setDateMethods() {
			...
			$init: function $init() {
				...
				this.data.scheme({
					$init: function $init(data) {
						if (typeof data.start_date == "string") data.start_date = webix.i18n.fullDateFormatDate(data.start_date);
						if (typeof data.end_date == "string") data.end_date = webix.i18n.fullDateFormatDate(data.end_date);
					},
			...
			syncData: function syncData(target, start, end, filter) {
			...
			_syncAllViews: function _syncAllViews() {
				this.coreData.callEvent("onChange", []);
				this.$$("multiDayList").getParentView().resize();
			},
			getWeekTitle: function getWeekTitle(date) {
				return scheduler.templates.week_title(date);
			},
			getTodayMultiEvents: function getTodayMultiEvents() {
			...
			getTodayEvents: function getTodayEvents() {
				var scheduler = this.getTopParentView();
				var d = webix.Date.datePart(scheduler.coreData.getValue(), true);
				var next = webix.Date.add(d, 1, "day", true);
				var filter = null;
	
				if (config.multi_day && scheduler.$$("dayList").isVisible()) filter = function filter(ev) {
					var dateStart = webix.Date.datePart(ev.start_date, true);
					var dateEnd = webix.Date.datePart(ev.end_date, true);
					return webix.Date.equal(dateStart, dateEnd) || ev.start_date.valueOf() > d.valueOf() || ev.end_date.valueOf() < next.valueOf();
				};
			...
			_setMonthFlags: function _setMonthFlags(old_Date, new_Date) {
				...
				while (start < end) {
					...
					for (var i = 0; i < events.length; i++) {
						if (events[i].start_date < next && events[i].end_date > start) top._eventsByDate[start.valueOf()] = true;
					}start = next;
				}
			},
			_initStructure: function _initStructure() {
				...
				} else if (config.readonly) this.define("readonly", true);
				...
				this.coreData.attachEvent("onChange", webix.bind(function () {
					...
					if (!this._isFormCalendarZoomed) {
						if (this._load_mode && this._load_url) this.load.apply(this, [this._load_url]);
			...
			_on_event_clicked: function _on_event_clicked(id) {
				this._last_selected = (id || "").toString().split("#");
				if (this._last_selected[1]) this.setCursor(null);
				this.setCursor(this._last_selected[0]);
				if (this.callEvent("onBeforeEventShow", [this._last_selected[0]])) this.$$('event').show();
			},
			/*Sorts dates asc, gets hash of dates with event*/
			_sortDates: function _sortDates() {
				this.blockEvent();
			...
			_initMonth: function _initMonth() {
				...
				this.$$("calendar").attachEvent("onAfterZoom", function (zoom) {
					if (!zoom) this.callEvent("onBeforeMonthChange", [null, this.config.date]);
				});
				this.$$("calendar").attachEvent("onBeforeMonthChange", webix.bind(function (oldDate, date) {
					if (!this._isFormCalendarZoomed) {
						var today = new Date();
						if (date.getMonth() === today.getMonth() && date.getYear() === today.getYear()) date = today;else date.setDate(1);
				...
				this.$$("calendar").config.dayTemplate = webix.bind(function (date) {
					if (this._eventsByDate && this._eventsByDate[date.valueOf()]) return this.$$("calendar").config.dayWithEvents(date);
					return date.getDate();
				}, this);
			},
			getEvents: function getEvents(from, to) {
				...
				for (var i = 0; i < evs.length; i++) {
					if (evs[i].start_date < to && evs[i].end_date > from) result.push(webix.copy(evs[i]));
				}
	
				return result;
			},
			_correctEventsDates: function _correctEventsDates(date, events) {
				var event;
				var result = [];
			...
			setDate: function setDate(date, inc, mode) {
				if (!date) date = this.coreData.getValue();else date = webix.Date.datePart(date, true);
	
				if (inc) date = webix.Date.add(date, inc, mode, true);
			...
			_initToolbars: function _initToolbars() {
	      	...
	      	this.remove(this.getCursor());*/
						...
						case "start_date":
						case "end_date":
							if (this.$$(view_id).name == "datetext") this._showDateForm(view_id);
							break;
						case "done":
							if (this._isFormCalendarZoomed) {
			...
			readonly_setter: function readonly_setter(val) {
				if (this.$$("add")) {
					if (val) {
			...
			_clearNoEventsStyle: function _clearNoEventsStyle() {
				if (this.dataCount()) this.$view.className = this.$view.className.replace(RegExp(this.type.cssNoEvents, "g"), "");else this.$view.className += " " + this.type.cssNoEvents;
			},
			_saveEvent: function _saveEvent() {
				//alert(this.$$("editForm").validate())
				if (this.$$("editForm").validate()) {
					if (!this.config.editEvent) {
						var data = this.$$("editForm").getValues();
						data.id = webix.uid();
						if (data.details && this._trim(data.text) === "" && this._trim(data.details) !== "") data.text = data.details;
						this.add(data);
	   	...
	   	this.$$("delete").hide();*/
			...
			eventDefaultDate: function eventDefaultDate() {},
			/* edit selected event*/
			_editEvent: function _editEvent() {
				if (this.$$("delete")) this.$$("delete").show();
				this.define("editEvent", true);
				this.$$("form").show();
				if (this.$$("rec_type")) this.$$("rec_type").setValue(this.$$("rec_type").getValue(), true);
				this._setDefaultDates();
			},
			/*cofirm the view changing (necessary for edit form)*/
			_confirmViewChange: function _confirmViewChange(button) {
				if (this.innerId(this.$$("views").getActiveId()) == "form") {
					var self = this;
					if (button != "today") webix.confirm({
						height: scheduler.xy.confirm_height,
						width: scheduler.xy.confirm_width,
						title: labels.icon_close,
			...
			_showDateForm: function _showDateForm(name) {
				this.dateField.setValue(name);
				if (this.$$("editForm").elements.allDay) this.$$("formCalendar").define("timepicker", !this.$$("editForm").elements.allDay.getValue());
				this.$$("formdate").show();
			...
			_setAllDay: function _setAllDay() {
				...
				for (var i = 0; i < 2; i++) {
					...
					if (elements.allDay.getValue() != 1) fields[i].setValue(this._event_date[i], true);
				}
				if (elements.allDay.getValue() == 1) {
					...
					if (end.valueOf() <= fields[0].getValue().valueOf()) end = webix.Date.add(end, 1, "day", true);
			...
			setLoadMode: function setLoadMode(mode) {
				if (mode == "all") mode = "";
			...
			load: function load(url) {
				...
				if (this._load_mode && typeof url == "string") {
					...
					if (this._loaded_ranges[lf(from)]) from = webix.Date.add(from, 1, this._load_mode, true);
...
/***/ function(module, exports) {
	...
	var values = exports.values = {
	    ...
	    calendar_hour: "%H:%i",
	    form_rules: {
	        end_date: function end_date(value, obj) {
	...
	function init() {
	    ...
	    if (!config.form) {
	        config.form = [{ view: "text", label: labels.label_event, labelWidth: 90, name: 'text' }, { view: "datetext", label: labels.label_start, labelWidth: 90, id: 'start_date', name: 'start_date', dateFormat: config.form_date }, { view: "datetext", label: labels.label_end, labelWidth: 90, id: 'end_date', name: 'end_date', dateFormat: config.form_date }, { view: "checkbox", id: 'allDay', name: 'allDay', label: labels.label_allday, labelWidth: 100, value: 0 }, { view: "textarea", label: labels.label_details, name: 'details', labelWidth: 90, height: 150 }, {}];
	    }
	    ...
	    if (!config.bottom_toolbar) {
	        config.bottom_toolbar = [{ view: "label", id: "today", name: "today", label: labels.icon_today, inputWidth: scheduler.xy.icon_today, align: "center", width: scheduler.xy.icon_today + 6 }, { view: "segmented", id: "buttons", value: config.mode || "week", align: "center", multiview: true, options: [{ id: "day", value: labels.day_tab, width: scheduler.xy.day_tab }, { id: "week", value: labels.week_tab, width: scheduler.xy.week_tab }, { id: "month", value: labels.month_tab, width: scheduler.xy.month_tab }] }, { view: "label", css: "add", name: "add", id: "add", align: "center", label: "&nbsp;+&nbsp;", width: 45 }, { view: "label", label: "", width: 45, batch: "readonly" }];
	    }
	    if (!config.day_toolbar) {
	        config.day_toolbar = [{ view: 'label', id: "prev", align: "left", label: "<div class='webix_cal_prev_button'><div></div></div>" }, { view: 'label', id: "date", align: "center", width: 200 }, { view: 'label', id: "next", align: "right", label: "<div class='webix_cal_next_button'><div></div></div>" }];
	    }
	    if (!config.week_toolbar) {
	        config.week_toolbar = [{ view: 'label', name: 'prevWeek', id: "prevWeek", width: 40, align: "left", label: "<div class='webix_cal_prev_button'></div>" }, { view: 'label', id: "weekTitle", align: "center" }, { view: 'label', id: "nextWeek", align: "right", label: "<div class='webix_cal_next_button'></div>", width: 40 }];
	    }
	    if (!config.selected_toolbar) {
	        config.selected_toolbar = [{ view: 'label', width: scheduler.xy.icon_back, css: "cancel", name: "back", id: "back", align: "center", label: labels.icon_back }, { view: 'button', inputWidth: scheduler.xy.icon_edit, name: "edit", id: "edit", align: "right", label: labels.icon_edit }];
	    }
	    if (!config.form_toolbar) {
	        config.form_toolbar = [{ view: 'label', width: scheduler.xy.icon_cancel, name: "cancel", id: "cancel", css: "cancel", align: "center", label: labels.icon_cancel }, { view: 'button', inputWidth: scheduler.xy.icon_save, name: "save", id: "save", align: "right", label: labels.icon_save }];
	    }
	    if (!config.date_toolbar) {
	        config.date_toolbar = [{ view: 'label', width: scheduler.xy.icon_cancel, name: "cancel_date", id: "cancel_date", css: "cancel", align: "center", label: labels.icon_cancel }, { view: 'label', id: "datetype", align: "center" }, { view: 'button', width: scheduler.xy.icon_done, name: "done", id: "done", align: "right", label: labels.icon_done }];
	    }
	}
...
/***/ function(module, exports) {
	...
	exports.__esModule = true;
	var values = exports.values = {
	    day_event_style: function day_event_style(obj) {
	        var style = "";
	        if (obj.color) {
	    ...
	    multi_day_event_style: function multi_day_event_style(obj) {
	        var style = "";
	        if (obj.color) {
	            var rgb = webix.color.toRgb(obj.color);
	    ...
	    selected_event: function selected_event(obj) {
	        var html = "",
	            fts = "",
	            fte = "";
	    ...
	    week_title: function week_title(date) {
	    ...
	    event_time: function event_time(obj) {
	        var start = obj.start_date;
	        var end = obj.end_date;
	        if (!(webix.Date.datePart(start, true).valueOf() == start.valueOf() && webix.Date.datePart(end, true).valueOf() == end.valueOf())) return webix.i18n.timeFormatStr(start);else return scheduler.locale.labels.label_allday;
	    },
	    event_marker: function event_marker(obj) {
	        var style = "";
	        if (obj.color) {
	            var rgb = webix.color.toRgb(obj.color);
	            style += ";border-color:" + obj.color + ";";
	    ...
	    event_title: function event_title(obj, type) {
	    ...
	    month_event_title: function month_event_title(obj, type) {
	        return "<div class='webix_event_overall " + type.eventClass(obj, type) + "'><div class='webix_event_time'>" + type.timeStart(obj) + "</div>" + type.marker(obj, type) + "<div class='webix_event_text' " + (obj.textColor ? "style='color:" + obj.textColor : "") + "'>" + obj.text + "</div></div>";
	    },
	    day_event: function day_event(obj, type) {
	        return obj.text;
	    },
	    multi_day_event: function multi_day_event(obj, type) {
	        return obj.text;
	    },
	    new_event_data: function new_event_data() {
	...
	var code = exports.code = function code() {
		...
		webix.attachEvent("onAfterSchedulerInit", function (scheduler) {
			...
			scheduler.getEvents = function (from, to) {
				...
				var out = [];
				for (var i = 0; i < evs.length; i++) {
					if (evs[i].rec_type) this.repeat_date(evs[i], out, from, to);else out.push(evs[i]);
				}
			...
			scheduler._editEvent = function () {
				...
				if ((item.rec_type || item.event_pid) && config.recurring_dialogs) {
					webix.rec_dialog({
						...
						callback: function callback(result) {
							if (result == 1) {
								if (item.event_pid) scheduler.setCursor(item.event_pid);
								scheduler._editEventBase.call(scheduler);
							} else if (result == 2) scheduler._applyRecExeption("edit");
						},
				...
				} else scheduler._editEventBase.call(this);
			};
			...
			scheduler._deleteEvent = function () {
				...
				if ((item.rec_type || item.event_pid) && config.recurring_dialogs) {
					webix.rec_dialog({
						...
						callback: function callback(result) {
							...
							} else if (result == 2) scheduler._applyRecExeption("remove");
						},
				...
				} else scheduler._deleteEventBase.call(this);
			};
			...
			scheduler._applyRecExeption = function (mode) {
				...
				if (mode == "edit") {
					...
					if (data.rec_type) {
						...
						if (this.$$("rec_type") && !this.$$("rec_type").config.hidden) this._recTypeState = "hide";
						...
						if (!id && !id[1]) return scheduler._editEventBase.call(this);
			...
			scheduler.data.scheme({
				$init: function $init(data) {
					...
					if (data.event_pid) scheduler._add_rec_marker(data, data.event_length * 1000);
				},
				$serialize: function $serialize(data) {
					var obj = {};
					obj.start_date = webix.i18n.fullDateFormatStr(webix.Date.copy(data.start_date));
				...
				if (!webix.Date[f]) {
					...
					if (str[0] == "day" || str[0] == "week") {
						...
						webix.Date[f] = function (nd, td) {
							...
							if (str[3]) scheduler.day_week(nd, str[2], str[3]);
						};
						webix.Date.add[type] = function (nd, inc) {
				...
				if (mode == "delete" || mode == "update" || mode == "insert" && !data.rec_type && data.event_pid) {
					...
					for (var i in sub) {
						if (sub.hasOwnProperty(i)) {
							id = sub[i].id;
							if (this.getItem(id)) this.remove(id);
						}
					}
					this.unblockEvent();
				}
			...
			scheduler.repeat_date = function (ev, stack, from, to) {
				...
				while (td < ev.start_date || td.valueOf() + ev.event_length * 1000 <= from.valueOf()) {
					...
					if (!ch) {
						...
						if (shift) {
							if (shift > 0) copy.end_date = new Date(td.valueOf() + ev.event_length * 1000 - shift * 60 * 1000);else {
		...
		webix.attachEvent("onBeforeSchedulerInit", function () {
			...
			if (!config.form) {
				config.form = [{ view: "text", label: labels.label_event, name: 'text', labelWidth: 90 }, { view: "datetext", label: labels.label_start, id: 'start_date', name: 'start_date', dateFormat: config.form_date, labelWidth: 90 }, { view: "datetext", label: labels.label_end, id: 'end_date', name: 'end_date', dateFormat: config.form_date, labelWidth: 90 }, { view: "checkbox", id: 'allDay', name: 'allDay', label: labels.label_allday, value: 0, labelWidth: 100 }, { view: "rectext", label: labels.recurring.repeat, id: 'rec_type', name: 'rec_type', readonly: true, labelWidth: 90 }, { view: "textarea", label: labels.label_details, id: 'details', name: 'details', height: 110, labelWidth: 90 }, { view: "text", hidden: true, id: 'event_length', name: 'event_length' }];
			}
	
			if (!config.recurring_bar) {
				config.recurring_bar = [{ view: 'label', width: xy.icon_cancel, id: "recCancel", name: "recCancel", css: "cancel", align: "center", label: labels.icon_cancel, batch: "default" }, { view: 'label', width: xy.icon_cancel, id: "recEndCancel", name: "recEndCancel", css: "cancel", align: "center", label: labels.icon_cancel, batch: "endRepeat" }, {}, { view: 'button', width: xy.icon_done, id: "recDone", name: "recDone", align: "right", label: labels.icon_done, batch: "default" }, { view: 'button', width: xy.icon_done, id: "recDone2", name: "recDone2", align: "right", label: labels.icon_done, batch: "other" }, { view: 'button', width: xy.icon_done, id: "recEndDone", name: "recEndDone", align: "right", label: labels.icon_done, batch: "endRepeat" }];
			}
			/*list of recurring types in the first view*/
			if (!config.recurring_views_list) config.recurring_views_list = [{ id: "none", value: labels.recurring.none, readonly: true }, { id: "daily", value: labels.recurring.daily }, { id: "weekly", value: labels.recurring.weekly }, { id: "monthly", value: labels.recurring.monthly }, { id: "yearly", value: labels.recurring.yearly }];
			...
			if (!config.rec_init_form) config.rec_init_form = [{ height: 10 }, { view: "reclist", id: "recList", autoheight: true, scroll: false, select: true, css: "rec_list", value: "none", template: scheduler.templates.rec_list_item, datatype: "json", data: config.recurring_views_list, labelWidth: 100 }, { maxHeight: 42 }, { view: "endrec", id: "endRepeat", name: "endRepeat", labelWidth: xy.recurring.label_end_repeat, label: labels.recurring.end_repeat }, {}];
			if (!config.end_repeat_form) config.end_repeat_form = [{
				type: "clean",
				paddingX: 10,
				paddingY: 5,
				cols: [{ view: "radio", css: "webix_endby_radio", vertical: true, id: "endBy", width: 290, labelWidth: xy.recurring.label_end_by, height: 80, options: [{ value: labels.recurring.endless_repeat, id: "0" }, { value: labels.recurring.end_repeat_label, id: "1" }] }, {}]
			}, { view: "calendar", id: "endByDate", width: 0, css: "end_rep_calendar", icons: false }, {}];
			/*Daily view*/
			if (!config.daily_form) config.daily_form = [{
				type: "clean",
				css: "webix_rec_counter",
				cols: [{ view: "label", label: labels.recurring.every, width: xy.recurring.label_every }, { view: "counter", id: "dayCount", name: "dayCount", value: 1, width: 125 }, { view: "label", label: labels.recurring.day }]
			}, {}];
	
			templates.selected_event_base = templates.selected_event;
			templates.selected_event = function (ev, common) {
				if (!ev.rec_type) return templates.selected_event_base(ev);else {
					var id = common.getTopParentView()._last_selected;
	
					if (!id || !id[1]) return templates.selected_event_base(ev);
			...
			if (!config.weekly_form) config.weekly_form = [{
				height: 10
			}, {
				type: "clean",
				css: "webix_rec_counter",
				paddingX: 8,
				cols: [{ view: "label", borderless: true, label: labels.recurring.every, width: xy.recurring.label_every }, { view: "counter", id: "weekCount", name: "weekCount", value: 1, width: 125 }, { view: "label", label: labels.recurring.week }]
			...
			for (var i = 1; i < 32; i++) {
				monthDays[i] = i;
			}if (!scheduler.templates.rec_month_d) scheduler.templates.rec_month_d = function () {
				return "<div class='radio'><div class='on'></div></div><div class='text'>" + labels.recurring.month_day + "</div>";
			};
			if (!scheduler.templates.rec_month_w) scheduler.templates.rec_month_w = function () {
				return "<div class='radio'><div class='on'></div></div><div class='text'>" + labels.recurring.week_day + "</div>";
			};
	
			if (!scheduler.templates.rec_month_d_blured) scheduler.templates.rec_month_d_blured = function () {
				return "<div class='radio blured'></div><div class='text'>" + labels.recurring.month_day + "</div>";
			};
			if (!scheduler.templates.rec_month_w_blured) scheduler.templates.rec_month_w_blured = function () {
				return "<div class='radio blured'></div><div class='text'>" + labels.recurring.week_day + "</div>";
			};
	
			if (!config.week_day_counters) config.week_day_counters = [{ id: 1, value: labels.recurring.counters[0] }, { id: 2, value: labels.recurring.counters[1] }, { id: 3, value: labels.recurring.counters[2] }, { id: 4, value: labels.recurring.counters[3] }, { id: 5, value: labels.recurring.counters[4] }];
			if (!config.week_day_labels) config.week_day_labels = [{ id: 1, value: webix.i18n.calendar.dayFull[1] }, { id: 2, value: webix.i18n.calendar.dayFull[2] }, { id: 3, value: webix.i18n.calendar.dayFull[3] }, { id: 4, value: webix.i18n.calendar.dayFull[4] }, { id: 5, value: webix.i18n.calendar.dayFull[5] }, { id: 6, value: webix.i18n.calendar.dayFull[6] }, { id: 7, value: webix.i18n.calendar.dayFull[0] }];
			if (!config.monthly_form) config.monthly_form = [{
				type: "clean",
				paddingX: 10,
				cols: [{ view: "label", label: labels.recurring.every, width: xy.recurring.label_every }, { view: "counter", id: "monthCount", name: "monthCount", value: 1, width: 125 }, { view: "label", label: labels.recurring.month }, { view: "text", hidden: true, name: "monthType", value: "Day", width: 1 }]
			}, {
				view: "accordion",
				id: "monthTypeSelect",
				//borderless: true,
				rows: [{
					...
					body: {
						...
						rows: [{ height: 10 }, {
							type: "clean",
							paddingX: 10,
							cols: [{ view: "select", value: 1, id: "weekCountM", name: "weekCountM", css: "", options: config.week_day_counters }, { view: "select", value: 1, id: "weekDayM", name: "weekDayM", css: "", options: config.week_day_labels }] }, {}]
					}
				}]
			}];
			...
			/*Yearly view*/
			if (!config.yearly_form) config.yearly_form = [{
				type: "clean",
				cols: [{ view: "label", label: labels.recurring.every, width: xy.recurring.label_every }, { view: "counter", id: "yearCount", name: "yearCount", value: 1, width: 125 }, { view: "label", label: labels.recurring.year }]
			}, {
				view: "datecells",
				id: "monthDayY",
				borderless: true,
				...
				id: "yearWeekRow",
				type: "clean",
				cols: [{ view: "select", value: 1, id: "weekCountY", name: "weekCountY", options: config.week_day_counters }, { view: "select", value: 1, id: "weekDayY", name: "weekDayY", options: config.week_day_labels }]
			}, {}];
	
			/*recurring subviews*/
			if (!config.recurring_views) config.recurring_views = [{ id: "recForm", view: "form", css: "webix_rec_form", padding: 0, scroll: false, elements: config.rec_init_form }, { id: "endRepeatForm", view: "form", type: "clean", elements: config.end_repeat_form }, { id: "daily", view: "daily", elements: config.daily_form }, { id: "weekly", view: "weekly", type: "clean", elements: config.weekly_form }, { id: "monthly", view: "monthly", elements: config.monthly_form, margin: 10 }, { id: "yearly", view: "yearly", padding: 10, margin: 10, type: "clean", elements: config.yearly_form }];
			...
			webix.protoUI({
				...
				defaults: {
					padding: 0,
					rows: [{ view: "toolbar", id: "reccuringBar", css: "webix_subbar", elements: config.recurring_bar, visibleBatch: "default" }, { id: "recViews", animate: false, cells: config.recurring_views }],
					...
					webix.delay(function () {
						...
						topParent.$$("reccuringBar").attachEvent("onItemClick", webix.bind(function (id) {
							...
							switch (id) {
								...
								case "recEndDone":
									var endDate;
									if (this.$$("endBy").getValue() == "1") endDate = this.$$("endByDate").getValue();else endDate = this.$$("recurring").config.endlessDate;
						...
						if (topParent.$$("typeWeekY")) topParent.$$("typeWeekY").attachEvent("onItemClick", function () {
							if (this.getValue()) topParent.$$("yearWeekRow").show();else topParent.$$("yearWeekRow").hide();
						});
						if (topParent.$$("endBy")) topParent.$$("endBy").attachEvent("onItemClick", function () {
							if (this.getValue() == "1") topParent.$$("endByDate").show();else topParent.$$("endByDate").hide();
						});
						...
						topParent.$$("views").attachEvent("onViewChange", function (v1, v2) {
							if (topParent._recTypeState && topParent.$$("form").config.id == v2) {
								...
								} else if (topParent._recTypeState == "show") topParent.$$("rec_type").show();
							}
						});
					}, this);
				},
				_showRecViewsList: function _showRecViewsList() {
					var topParent = this.getTopParentView();
				...
				_showRecView: function _showRecView(id) {
					...
					if (!this.$$("endRepeat").getValue()) this.$$("endRepeat").setValue(this.$$("recurring").config.endlessDate);
				},
				...
				_applyRecurrence: function _applyRecurrence() {
					var topParent = this.getTopParentView();
					...
					if (!value[0]) {
						...
						if (value[5]) startDate.setDate(value[5]);
					...
					value.pop();
					var repeat = "";
					if (topParent.$$("endBy").getValue() == "0") repeat = "no";
					topParent.$$("rec_type").setValue(value.join("_") + "#" + repeat);
				},
				_setEndDate: function _setEndDate() {
					var topParent = this.getTopParentView();
				...
				_setEndByVisibility: function _setEndByVisibility() {
					var topParent = this.getTopParentView();
					var value = topParent.$$("endBy").getValue();
					if (value == "1") topParent.$$("endByDate").show();else topParent.$$("endByDate").hide();
				}
...
/***/ function(module, exports) {
	...
	webix.protoUI({
		...
		_toHTML: function _toHTML(obj) {
			var mark = this.data.getMark(obj.id);
		...
		_renderScale: function _renderScale() {
			var html = "<div></div>";
		...
		on_click: {},
		hourScaleItem: function hourScaleItem(hour) {
		...
		_prepareEvents: function _prepareEvents() {
				...
				if (!_sorder_set) {
					if (stack.length) {
						if (stack.length <= stack[stack.length - 1].$sorder) {
							if (!stack[stack.length - 1].$sorder) ev.$sorder = 0;else for (j = 0; j < stack.length; j++) {
								_is_sorder = false;
							...
							for (j = 1; j < stack.length; j++) {
								if (stack[j].$sorder > _max_sorder) _max_sorder = stack[j].$sorder;
			...
			for (var _i = 0; _i < evs.length; _i++) {
				evs[_i].$count = stack.max_count;
				this._setPosition(evs[_i]);
			}
		},
		_setPosition: function _setPosition(ev) {
		...
		_getEventEndDate: function _getEventEndDate(ev) {
...
/***/ function(module, exports) {
	...
	webix.protoUI({
	    ...
	    _after_render: function _after_render(obj) {
	        if (webix.isUndefined(obj.value)) return;
	        this.$setValue(obj.value);
	    },
	    setValue: function setValue(value, forceReset) {
	        ...
	        if (this.getInputNode()) this.$setValue(value);
	
	        if (changed) this.callEvent("onChange", [value, oldvalue]);
	    },
	    $setValue: function $setValue(value) {
	        this.config.value = value || "";
	        var format = this.config.dateFormatStr || webix.i18n.dateFormatStr;
	        this.config.text = this.getInputNode().innerHTML = _typeof(this.config.value) == "object" ? format(this.config.value) : this.config.value;
	    },
	    dateFormat_setter: function dateFormat_setter(value) {
	...
	webix.protoUI({
	    ...
	    render: function render(id, data) {
	        ...
	        if (this.callEvent("onBeforeRender", [this.data])) {
	            ...
	            if (this.units) {
	                ...
	                if (!data.length) this.$view.firstChild.innerHTML = "<div class='webix_noevents'>" + this.config.noEvents + "</div>";else this.$view.firstChild.innerHTML = data.map(this._toHTML, this).join("");
	            }
	            this.callEvent("onAfterRender", []);
	        }
	    },
	    ...
	    _setUnits: function _setUnits() {
	        ...
	        while (start.valueOf() < end.valueOf()) {
	            ...
	            if (result.length) this.units[start.valueOf()] = result;
	    ...
	    type: {
	        ...
	        templateStart: function templateStart(obj, type) {
	            ...
	            return "<div webix_l_id='" + obj.id + "' class='" + className + "' style='" + style + "'>";
	        },
	        templateStartHeader: function templateStartHeader(obj, type) {
...
/***/ function(module, exports) {
	...
	webix.protoUI({
	    ...
	    on_click: {
	        webix_cal_day_num: function webix_cal_day_num(e, id) {
	            if (id) this.define("activeCell", id);
	        }
	    },
	    activeCell_setter: function activeCell_setter(id) {
	    ...
	    render: function render() {
	        if (this.isVisible(this.config.id)) {
	            ...
	            for (var i in obj) {
	                ...
	                if (c % this.config.x_count == 1) html += "<tr>";
	                html += "<td style='" + (cellWidth ? "width:" + cellWidth + "px" : "") + "'>";
	
	                if (this.config.activeCell == i) className += " webix_cal_selected_day";
	
	                html += "<div webix_d_c='" + i + "' class='" + className + "' style='" + style + "'>" + obj[i] + "</div>";
	                html += "</td>";
	                if (c % this.config.x_count === 0) html += "</tr>";
	                c++;
	            }
...
/***/ function(module, exports) {
	...
	webix.protoUI({
	    ...
	    $setValue: function $setValue(value) {
	        ...
	        if (arr[0] !== "") type = recView.config.typeToId[arr[0]];
...
/***/ function(module, exports) {
	...
	webix.protoUI({
	    ...
	    setValue: function setValue(value) {
	        ...
	        if (this.getInputNode()) this.$setValue(value);
	        if (!skipEvent) this.callEvent("onChange", [value, oldvalue]);
	    },
	    $setValue: function $setValue(date) {
	        ...
	        if ((typeof date === "undefined" ? "undefined" : _typeof(date)) == "object") {
	            if (date.valueOf() == recView.config.endlessDate.valueOf()) text = scheduler.locale.labels.recurring.never;else text = (this.config.dateFormatStr || webix.i18n.dateFormatStr)(this.config.value);
	        }
...
/***/ function(module, exports) {
	...
	webix.protoUI({
	    ...
	    setValue: function setValue(value) {
	        if (this.getItem(value)) this.select(value);
	    },
	    getValue: function getValue() {
...
/***/ function(module, exports) {
	...
	webix.protoUI({
	    name: "daily",
	    setValue: function setValue(data) {
	        if ((typeof data === "undefined" ? "undefined" : _typeof(data)) == "object" && data[0] == "day") {
	            if (!data[1] || data[1] === "") data[1] = 1;
...
/***/ function(module, exports) {
	...
	webix.protoUI({
	    name: "weekly",
	    setValue: function setValue(data) {
	        if ((typeof data === "undefined" ? "undefined" : _typeof(data)) == "object" && data[0] == "week") {
	            if (!data[1] && data[1] === "") data[1] = 1;
	            if (!data[4] || data[4] === "") data[4] = (data[5].getDay() + 6) % 7 + 1;
	    ...
	    getValue: function getValue() {
	        var data = this.getValues();
	        if (!data.weekDays) data.weekDays = 1;
...
/***/ function(module, exports) {
	...
	webix.protoUI({
	    ...
	    defaults: {
	        ...
	        rows: [{ view: "checkbox", name: 1, id: 1, labelRight: webix.i18n.calendar.dayFull[1] }, { view: "checkbox", name: 2, id: 2, labelRight: webix.i18n.calendar.dayFull[2] }, { view: "checkbox", name: 3, id: 3, labelRight: webix.i18n.calendar.dayFull[3] }, { view: "checkbox", name: 4, id: 4, labelRight: webix.i18n.calendar.dayFull[4] }, { view: "checkbox", name: 5, id: 5, labelRight: webix.i18n.calendar.dayFull[5] }, { view: "checkbox", name: 6, id: 6, labelRight: webix.i18n.calendar.dayFull[6] }, { view: "checkbox", name: 7, id: 7, labelRight: webix.i18n.calendar.dayFull[0] }]
	    },
	    getValue: function getValue() {
	        var values = this.getValues();
	        var result = [];
	        for (var d in values) {
	            if (values[d]) result.push(d);
	        }return result.join();
	    },
	    setValue: function setValue(values) {
	        ...
	        for (var i = 1; i < 8; i++) {
	            result[i] = 0;
	        }if ((typeof values === "undefined" ? "undefined" : _typeof(values)) != "object") values = values.split(",");
	        for (var _i = 0; _i < values.length; _i++) {
	            result[parseInt(values[_i], 10)] = 1;
...
/***/ function(module, exports) {
	...
	webix.protoUI({
	    ...
	    setValue: function setValue(data) {
	        if ((typeof data === "undefined" ? "undefined" : _typeof(data)) == "object" && data[0] == "month") {
	            var topParent = this.getTopParentView();
	            if (!data[1] && data[1] === "") data[1] = 1;
	            if (data[2] && data[2] !== "") topParent.$$("typeWeekM").expand();else {
...
/***/ function(module, exports) {
	...
	webix.protoUI({
	    ...
	    setValue: function setValue(data) {
	        if ((typeof data === "undefined" ? "undefined" : _typeof(data)) == "object" && data[0] == "year") {
	            var topParent = this.getTopParentView();
	            if (!data[1] && data[1] === "") data[1] = 1;
	            if (data[2] && data[2] !== "") topParent.$$("typeWeekY").setValue(1);else {
...
/***/ function(module, exports) {
	...
	webix.protoUI({
	    ...
	    defaults: {
	        ...
	        body: {
	            type: "clean",
	            rows: [{ id: "webix_confirm_message", template: "<div class='webix_dialog_text'>#text#</div>",
	                data: { text: "You have forgot to define the text :) " }
	            }, { height: 44, view: "button", label: "Series", click: function click() {
	                    this.getParentView().getParentView()._callback(1);
	                }
	            }, { height: 44, view: "button", label: "Occurrence", click: function click() {
	                    this.getParentView().getParentView()._callback(2);
	                }
	            }, { height: 44, view: "button", css: "cancel", label: "Cancel", click: function click() {
	                    this.getParentView().getParentView()._callback(false);
	                }
	            }]
	        }
	    },
	    labelSeries_setter: function labelSeries_setter(value) {
	        var body = this._body_cell._cells[1];
	        body.config.label = value;
	        body.render();
	    },
	    labelEvent_setter: function labelEvent_setter(value) {
	        var body = this._body_cell._cells[2];
	        body.config.label = value;
	        body.render();
	    },
	    labelCancel_setter: function labelCancel_setter(value) {
